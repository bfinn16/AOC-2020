...
/*
 Not my finest work
 NOTE TO SELF: Don't do these at midnight
*/

func parse(input: String) -> [[String: String]] {
    let rawPassports = input.components(separatedBy: "\n\n")
    let passports = rawPassports.map { ($0.split(separator: " ").map { $0.split(separator: "\n") }).flatMap { $0 } }
    var passportDict = [[String: String]]()
    for passport in passports {
        var elementDict = [String: String]()
        for element in passport {
            let stringElement = String(element)
            let passportElements = stringElement.split(separator: ":")
            elementDict[String(passportElements[0])] = String(passportElements[1])
        }
        passportDict.append(elementDict)
    }

    return passportDict
}

func verify(_ passports: [[String: String]], requiredFields: [String]) -> Int {
    var validPassports = 0
    for passport in passports {
        var isValidPassport = true
            for key in requiredFields {
                if passport.keys.count < 8 {
                    if passport[key] == nil {
                ...
                if !isValid(for: key, value: passport[key]!) {
                    isValidPassport = false
                    break
                }
        }
...
func isValid(for key: String, value: String) -> Bool {
    switch key {
    case "byr":
    ...
    case "hgt":
        ...
        let height = Int(value.prefix(value.count - 2))!
        if units == "cm" {
    ...
    case "hcl":
        return value.first == "#" && value.range(of: "[^a-f0-9]", options: .regularExpression) != nil && value.count == 7
    case "ecl":
        return
            value == "amb" ||
            value == "blu" ||
...
let input = parse(input: rawInput)
print(verify(input, requiredFields: ["ecl", "pid", "eyr", "hcl", "byr", "iyr", "hgt"]))
