...
struct Password {
    let min: Int
    let max: Int
    let keyLetter: Character
    let text: String

    init(_ input: String) {
        let splitInput = input.split(separator: " ")
        self.min = Int(splitInput[0].split(separator: "-")[0]) ?? -1
        self.max = Int(splitInput[0].split(separator: "-")[1]) ?? -1
        self.keyLetter = Character(String(splitInput[1].prefix(1)))
        self.text = String(splitInput[2])
    }
}

func isValidOne(_ input: String) -> Bool {
    let password = Password(input)
    let keyFrequency = password.text.reduce(0, { $0 + ($1 == password.keyLetter ? 1 : 0)})
    return (password.min...password.max).contains(keyFrequency)
}

func isValidTwo(_ input: String) -> Bool {
    let password = Password(input)
    return (password.text[password.min-1] == password.keyLetter) ^
    (password.text[password.max-1] == password.keyLetter)
}

func test(passwords: [String], isValid: (String) -> Bool) -> Int {
    return passwords.reduce(0, { $0 + (isValid($1) ? 1 : 0) })
}
...
print(test(passwords: testInput, isValid: isValidOne(_:)))
print(test(passwords: testInput, isValid: isValidTwo(_:)))
